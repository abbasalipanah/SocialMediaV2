...
BRAND_ID="${SOCIAL_X_CANARY_BRAND_ID:-18}"

# shellcheck source=platform_canary_env.sh
source "$ROOT/scripts/dev/platform_canary_env.sh"
prepare_platform_canary_env "$ROOT"

if [[ "${SOCIAL_X_ACCOUNT_ENABLED:-false}" != "true" ]]; then
  echo "X canary credential is not configured." >&2
  exit 1
fi
if [[ ! "$BRAND_ID" =~ ^[1-9][0-9]*$ ]]; then
  echo "SOCIAL_X_CANARY_BRAND_ID must be a positive integer." >&2
  exit 1
fi
if [[ ! -x "$PYTHON" ]]; then
...
if [[ "$SOCIAL_LOCAL_DB_ENV_FILE" != "$ROOT/.local/platform-expansion-db.env" ]] \
  ...
  echo "X canary collection requires the isolated platform-expansion database." >&2
  exit 1
fi
