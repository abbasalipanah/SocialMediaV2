    ...
    PlatformId.FACEBOOK: frozenset(
        {
            ...
            MetricId.FOLLOWERS_NET.value,
            MetricId.FOLLOWS.value,
            "frequency",
            "hashtag_comments",
            "hashtag_engagements",
            "hashtag_engagements_per_post",
            "hashtag_likes",
            "hashtag_posts",
            "hashtag_saves",
            "hashtag_shares",
            ...
            "page_like_types",
            "page_like_types_organic",
            "page_like_types_paid",
            MetricId.PAGE_VIEWS.value,
            "post_engagements_angry",
            "post_engagements_care",
            "post_engagements_comment",
            "post_engagements_haha",
            "post_engagements_like",
            "post_engagements_love",
            "post_engagements_pride",
            "post_engagements_sad",
            "post_engagements_share",
            "post_engagements_thankful",
            "post_engagements_wow",
            MetricId.REACH.value,
            MetricId.REACH_ORGANIC.value,
            MetricId.REACH_PAID.value,
            "reaction_angry",
            "reaction_care",
            "reaction_haha",
            "reaction_like",
            ...
            "saves",
            "shares",
            MetricId.TOTAL_ACTIONS.value,
            MetricId.UNFOLLOWS.value,
            "video_avg_time_ms",
            "video_view_time_ms",
            "video_views",
            MetricId.VIEWS.value,
            MetricId.VIEWS_ORGANIC.value,
...
_SUPPORTED_CANONICAL_METRICS_BY_PLATFORM = {
    platform: frozenset(
        definition.metric_id
        for definition in bootstrap_metric_catalog().definitions()
...
_AUDIENCE_PROJECTIONS: dict[str, tuple[MetricId, str]] = {
    ...
    "audience_countries": (MetricId.FOLLOWERS, "audience_countries"),
    "audience_city": (MetricId.FOLLOWERS, "audience_city"),
    "audience_cities": (MetricId.FOLLOWERS, "audience_cities"),
    "audience_gender_age": (MetricId.FOLLOWERS, "audience_gender_age"),
    "audience_ages": (MetricId.FOLLOWERS, "audience_ages"),
    "audience_genders": (MetricId.FOLLOWERS, "audience_genders"),
...
_LEGACY_DIRECT_ALIASES: dict[tuple[PlatformId, str], MetricId] = {
    ...
    (PlatformId.TIKTOK, "likes"): MetricId.VIDEO_LIKES_DAILY,
    (PlatformId.TIKTOK, "comments"): MetricId.VIDEO_COMMENTS_DAILY,
    (PlatformId.TIKTOK, "shares"): MetricId.VIDEO_SHARES_DAILY,
}
...
_FIXED_BREAKDOWNS: dict[tuple[PlatformId, str], tuple[MetricId, str, str]] = {
    ...
    (PlatformId.FACEBOOK, "page_like_types_paid"): (MetricId.FOLLOWERS, "page_like_type", "paid"),
}

_FACEBOOK_ENGAGEMENT_PREFIX = "post_engagements_"
_INSTAGRAM_INTERACTION_METRICS = frozenset({"comments", "likes", "replies", "saves", "shares"})
...
def legacy_metric_disposition(
    ...
    if platform is PlatformId.FACEBOOK and raw_metric_id.startswith(_FACEBOOK_ENGAGEMENT_PREFIX):
        return LegacyMetricDisposition.DASHBOARD_BREAKDOWN
    if (
    ...
    if platform is PlatformId.INSTAGRAM and raw_metric_id in {
        ...
        return LegacyMetricDisposition.DASHBOARD_BREAKDOWN
    try:
        metric_id = MetricId(raw_metric_id)
    except ValueError:
        known = raw_metric_id in KNOWN_LEGACY_METRIC_IDS_BY_PLATFORM.get(platform, frozenset())
        return (
            LegacyMetricDisposition.DASHBOARD_UNUSED if known else LegacyMetricDisposition.UNKNOWN
        )
    if metric_id not in _SUPPORTED_CANONICAL_METRICS_BY_PLATFORM[platform]:
        # MetricId is a global vocabulary, but support remains platform-specific.
        # A canonical identifier copied from another platform must not reach a
        # dashboard catalog that deliberately has no definition for that pair.
        return (
            LegacyMetricDisposition.DASHBOARD_UNUSED
            if raw_metric_id
            in KNOWN_LEGACY_METRIC_IDS_BY_PLATFORM.get(platform, frozenset())
...
def project_legacy_metrics(
    ...
    for raw in rows:
        disposition = legacy_metric_disposition(raw.platform, raw.raw_metric_id, raw.breakdown_key)
        ...
        keep(
            _reporting_metric(raw, metric_id, raw.breakdown_key, raw.breakdown_value),
