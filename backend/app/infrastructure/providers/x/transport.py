"""Allowlisted, bounded, read-only transport for X API v2."""
...
class XTransportError(RuntimeError):
    """Provider failure without tokens or response bodies."""
...
class XHttpTransport:
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or max_retries > 10
            or request_budget < 1
            or request_budget > 1_000
        ):
            raise XTransportError("x_transport_config_invalid")
    ...
    def get(
        ...
        *,
        ...
        if url not in self._get_urls:
            raise XTransportError("x_provider_url_rejected")
        if not access_token.strip():
            raise XTransportError("x_provider_access_token_invalid")
        ...
        *,
        ...
        while True:
            if self._remaining_requests < 1:
                raise XTransportError("x_provider_request_budget_exhausted")
            ...
            try:
                ...
                if attempt >= self._max_retries:
                    raise XTransportError("x_provider_transport_failed") from exc
    ...
    def _payload(response: httpx.Response) -> Mapping[str, object]:
        if response.status_code < 200 or response.status_code >= 300:
            raise XTransportError("x_provider_http_rejected")
        if len(response.content) > MAX_RESPONSE_BYTES:
            raise XTransportError("x_provider_response_too_large")
        try:
            payload: Any = response.json()
        except ValueError as exc:
            raise XTransportError("x_provider_response_invalid") from exc
        if not isinstance(payload, Mapping):
            raise XTransportError("x_provider_response_invalid")
        return payload
...
__all__ = ["XHttpTransport", "XTransportError"]
